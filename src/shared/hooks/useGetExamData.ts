...
export const useGetExamData = (windowHash: string, isOpenModal: boolean) => {
        ...
        const examIds = useMemo(() => {
            const idsSSS = data?.map(el => el.block_id?.toString()) || [] as string[]
        ...
        const audioData = useMemo(() => {
            return data?.reduce((acc, el, i) => {
                if (el.type === 'voice') {
                    return [...acc, {id: i,type: 'audio', audioName: el.audio_name ?? ''}]
            }
            return acc
        }, [] as IVoice[])
        }, [data])
        ...
        return{
            examIds, data, isLoading, error, audioData
        }
